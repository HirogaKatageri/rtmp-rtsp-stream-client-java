...
public abstract class DisplayBase implements GetAacData, GetVideoData, GetMicrophoneData {
    ...
    protected abstract void startStreamRtp(String url, String url2, String url3);
    ...
    public void startStream(String url, String url2, String url3) {
        streaming = true;
        if (!recordController.isRunning()) {
            startEncoders(resultCode, data);
        } else {
    ...
    private void resetVideoEncoder(boolean reset) {
        ...
        if (reset) videoEncoder.reset();
        else videoEncoder.forceKeyFrame();
        if (glInterface != null) {
            glInterface.addMediaCodecSurface(videoEncoder.getInputSurface());
        }
